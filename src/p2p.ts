      ...
      try {
        ...
        socket.on("data", async (data: Buffer) => {
          ...
          while (buffer.includes("\n")) {
            ...
            try {
              const response: P2PMessage = JSON.parse(line);
              log(`[sendP2PInject] Parsed response type: ${response.type}, requestId: ${response.requestId?.slice(0, 8) || 'none'}`);
              ...
              if (response.type === "response" && response.requestId === requestId) {
                ...
                log(`[sendP2PInject] Decrypted response (${decryptedResponse.length} chars): ${decryptedResponse.slice(0, 100)}...`);
      ...
      try {
        ...
        if (msg.type === "log") {
          ...
          if (onInjectMessage) {
            ...
            onLog(`[handleConnection] AI response received (${aiResponse.length} chars): ${aiResponse.slice(0, 100)}...`);
            ...
            onLog(`[handleConnection] Encrypting response, msg.v=${msg.v}, msg.ephemeralPub=${msg.ephemeralPub ? 'yes' : 'no'}, msg.encryptPub=${msg.encryptPub ? 'yes' : 'no'}`);
            ...
            if (msg.v >= 2 && msg.ephemeralPub) {
              ...
              onLog(`[handleConnection] Using ephemeral encryption, generated key: ${responseEphemeral.publicKey.slice(0, 16)}..., sender ephemeral: ${msg.ephemeralPub.slice(0, 16)}...`);
            ...
            onLog(`[handleConnection] Sending response message (${responseJson.length} chars), requestId: ${msg.requestId?.slice(0, 8)}...`);
