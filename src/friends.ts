...
export async function loadFriendsData(): Promise<void> {
	...
	const pendingRepo = _storage.getRepository<P2PPendingRequestRow>(
		"p2p",
		"pending_requests",
	);
	const autoAcceptRepo = _storage.getRepository<P2PAutoAcceptRow>(
		"p2p",
		"auto_accept",
	);
...
function serializeForSigning(
	obj: Omit<FriendRequest, "sig"> | Omit<FriendAccept, "sig">,
): string {
...
export function createFriendRequest(to: string, from: string): FriendRequest {
	...
	const signature = sign(
		null,
		Buffer.from(serializeForSigning(payload)),
		privateKey,
	);
 ...
 */
export function createFriendAccept(
...
export function storePendingRequest(
	...
	state.pendingOut = state.pendingOut.filter(
		(p) => p.request.to !== request.to,
	);
...
export function queueForApproval(
	...
	state.pendingIn = state.pendingIn.filter(
		(p) => p.request.from !== request.from,
	);
 ...
 */
export function getPendingOutgoing(
	to: string,
): OutgoingFriendRequest | undefined {
	const state = loadFriendsState();
 ...
 */
export function completeFriendship(
	accept: FriendAccept,
	channel: string,
): Friend {
	const state = loadFriendsState();
	...
	const pendingIdx = state.pendingOut.findIndex(
		(p) => p.request.to === accept.from,
	);
	if (pendingIdx !== -1) {
 ...
 */
export function acceptPendingRequest(
	from: string,
): { friend: Friend; request: FriendRequest } | null {
	const state = loadFriendsState();

	const pendingIdx = state.pendingIn.findIndex(
		(p) => p.request.from.toLowerCase() === from.toLowerCase(),
	);
	...
	// Check if already friends
	const existingIdx = state.friends.findIndex(
		(f) => f.publicKey === request.pubkey,
	);
	if (existingIdx === -1) {
		state.friends.push(friend);
	}
 ...
 */
export function getPendingIncomingBySignature(
	sig: string,
): PendingFriendRequest | undefined {
	const state = loadFriendsState();
...
export function denyPendingRequest(fromOrSig: string): boolean {
	...
	const idx = state.pendingIn.findIndex(
		(p) =>
			p.request.from.toLowerCase() === fromOrSig.toLowerCase() ||
			p.request.sig === fromOrSig,
	);
...
export function cleanupExpiredRequests(): void {
	...
	if (
		state.pendingIn.length !== originalInCount ||
		state.pendingOut.length !== originalOutCount
	) {
		saveFriendsState(state);
	}
}
